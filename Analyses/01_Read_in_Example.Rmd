...
#In this demo, I have not added all the meshes from the sample into github because that would cause it to clog. But it is a good idea to paste the meshes into a folder inside the repo, without actually pushing them (write the file extension into the gitignore file to avoid pushing - e.g. *.ply). It means that your collaborators all have to copy the _identical_ meshes into the identically-named folder on github, but allows you to visualise any mesh for things like quality control.
...
# if you are using patch/curve semilandmarks, use the above to figure out what numbers they are. I normally enter them into csv files manually. I have example csv files in the Data/Raw folder that do not match this dataset,just fyi. 
...
#We won't do it here, but you can use the mean shape configuration to warp the mesh of the specimen closest to the mean 

Ref <- mshape(GPA$coords)

#You can also now check if you have any outliers in your data. But don't rely on this alone, it won't pick up small but important mistakes! You should also check if the Csizes work (below) and ideally also use plotReftoTarget to check if all of your specimens have their landmarks correctly placed. I can send you some code to do this if you like.
...
#Here are the PC scores for each PC for each species
PCA$x
...
#Or you just save the things you really need for the future: 
save(GPA, PCA, speclist, file="../Data/Processed/GPA_PCA_Classifier.rda")
